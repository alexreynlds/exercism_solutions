...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    # Use the percentage to gauge the efficiency
    if percentage >= 80:
        return "green"
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    # Put current in variable for efficiency
    current = temperature * neutrons_produced_per_second

    # Using the current, test against threshold to get output
    if current < (threshold * 0.9):
        return "LOW"
    elif current <= (threshold * 1.10) and current >= (threshold * 0.9):
        return "NORMAL"
    else:
        return "DANGER"
